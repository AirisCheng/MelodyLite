...
import { download,setCollect,getCollectOfUserId, deleteCollect } from '../api/index'; // 导入 deleteCollect
...
export default {
    ...
    methods: {
        ...
        collection() {
            if(this.loginIn){
                if (this.isActive) { // 如果已经收藏，则取消收藏
                    deleteCollect(this.userId, this.id)
                        .then(res => {
                            if (res) { // deleteCollect 返回的是 boolean
                                this.$store.commit('setIsActive', false);
                    ...
                    setCollect(params)
                        .then(res =>{
                            if(res.code == 1){
                                ...
                                this.notify('已收藏','warning'); // 理论上这里不会出现，因为我们先判断了 isActive
                            }else{
